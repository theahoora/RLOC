...
from ourmodel import MemoryDNN
...
####################################################################################################
    np.save('theirs',df.rolling(rolling_intv, min_periods=1).mean().to_numpy()) #ahoora's plot check
####################################################################################################
...
if __name__ == "__main__":
    ...
    for i in range(n):
        ...
        # print('prob',m_pred)
        for i in range(K):
            new_instance = np.random.binomial(1,m_pred)
            found = False

            for a in m_list:
                if np.all(new_instance==a):
                    found = True
                    break

            if not found:
                m_list.append(new_instance)
        r_list = []
        r_mean = 0
        weight_sum = 0
        for m in m_list:
            r_list.append(bisection(h/1000000, m)[0])
            prob = (r_list[-1]*m_pred)+((1-r_list[-1]*(1-m_pred)))
            prob = np.sum(prob)
        ...
        for i in range(len(r_list)):
            mem.encode(h, m_pred,m_list[i],r_list[i],r_mean)
